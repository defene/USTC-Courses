...
void addNewCmd(	unsigned char *name, 
    ...
    */
    ...
    new_cmd->description = (char*) malloc((strlen(description) + 1) * sizeof(char));
...
int help(int argc, unsigned char **argv){
    ...
    while (tmpCmd != NULL) {            
        if (strcmp(argv[1],tmpCmd->name)==0) {
